...
tpm = np.load('tpm.npy')
naive_model = NaiveModel(len(input_cols), input_length, output_length=output_len)
...
for i in range(num_sims):
    ...
    while t < 100:
        ...
        # next_winds = naive_model(prev_winds)
        ...
        ind += lookahead
    error = np.sqrt(np.mean(np.square(xs)))
    if error < 1:
        arr.append(error)
    mean_error += error
